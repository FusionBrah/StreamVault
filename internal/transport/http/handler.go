...
func groupV1Routes(e *echo.Group, h *Handler) {
	...
	authGroup.POST("/api-key", h.GenerateAPIKey, AuthGuardMiddleware, AuthGetUserMiddleware)
	authGroup.GET("/api-key", h.GetAPIKey, AuthGuardMiddleware, AuthGetUserMiddleware)
	authGroup.DELETE("/api-key", h.DeleteAPIKey, AuthGuardMiddleware, AuthGetUserMiddleware)

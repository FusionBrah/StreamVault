...
// GenerateAPIKey generates a new API key for the user, replacing any existing key.
func (s *Service) GenerateAPIKey(ctx context.Context, userID uuid.UUID) (string, error) {
	...
	// Create key with sv_ prefix for easy identification
	apiKey := "sv_" + hex.EncodeToString(bytes)

	// Store the key in the database
	_, err := s.Store.Client.User.UpdateOneID(userID).SetAPIKey(apiKey).Save(ctx)
	if err != nil {
...
// GetAPIKey retrieves the current API key for a user (masked for security).
func (s *Service) GetAPIKey(ctx context.Context, userID uuid.UUID) (string, error) {
	u, err := s.Store.Client.User.Query().Where(entUser.ID(userID)).Only(ctx)
	if err != nil {
		return "", fmt.Errorf("error getting user: %v", err)
	}
	...
	// Return masked key: sv_xxxx...xxxx (show first 7 and last 4 chars)
	key := *u.APIKey
	if len(key) > 11 {
		return key[:7] + "..." + key[len(key)-4:], nil
	}
	return key, nil
}

// DeleteAPIKey removes the API key for a user.
func (s *Service) DeleteAPIKey(ctx context.Context, userID uuid.UUID) error {
	_, err := s.Store.Client.User.UpdateOneID(userID).ClearAPIKey().Save(ctx)
	if err != nil {
...
func (s *Service) OAuthUserCheck(ctx context.Context, userClaims OIDCCLaims) (*ent.User, error) {
	...
	if err != nil {
        ...
        for _, group := range userClaims.Groups {
            if strings.HasPrefix(group, "streamvault-") {
                groupRole := strings.TrimPrefix(group, "streamvault-")
                if utils.IsValidRole(groupRole) {
                    log.Debug().Msgf("Found StreamVault role in user group %v", group)

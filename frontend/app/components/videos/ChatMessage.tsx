...
import { Comment, StreamVaultFormattedBadge, StreamVaultFormattedMessageFragment, StreamVaultFormattedMessageType } from "@/app/hooks/useChat";
  ...
  return (
    <div key={comment._id} className={classes.chatMessage}>
      ...
      <span>
        {comment.ganymede_formatted_badges &&
          comment.ganymede_formatted_badges.map(
            (badge: StreamVaultFormattedBadge) => (
              badge.url && (
      ...
      {comment.ganymede_formatted_message && comment.ganymede_formatted_message.map(
        (fragment: StreamVaultFormattedMessageFragment) => {
          switch (fragment.type) {
            case StreamVaultFormattedMessageType.Text:
              return (
                <Text className={classes.message} span>
                  {fragment.text}
                </Text>
              )
            case StreamVaultFormattedMessageType.Emote:

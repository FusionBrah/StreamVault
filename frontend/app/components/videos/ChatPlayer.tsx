...
import {
  ...
  Comment,
  StreamVaultFormattedMessageFragment,
  StreamVaultFormattedMessageType,
  getChatForVideo,
  getSeekChatForVideo
...
const ChatPlayer = ({ video }: Params) => {
  ...
  const createSystemMessage = useMemo(() => (message: string): Comment => ({
    ...
    ganymede_formatted_message: [{
      type: StreamVaultFormattedMessageType.Text,
      text: message
    }]
  }), []);
  ...
  const addEmotesToFormattedComment = useCallback((comment: Comment): StreamVaultFormattedMessageFragment[] => {
    ...
    try {
      return comment.message.fragments.flatMap(fragment => {
        if (fragment.emoticon) {
          ...
          return [{
            type: StreamVaultFormattedMessageType.Emote,
            text: fragment.text,
      ...
      return [{ type: StreamVaultFormattedMessageType.Text, text: comment.message.body }];
    }
  }, [handleError]);
